				...
				*/
				if (atomic_read(&emmc_reboot)) {
					...
					if ((brq->data.flags & MMC_DATA_WRITE) != 0) {
						...
						spin_unlock_irq(&md->lock);
						mmc_release_host(card->host);
						pr_info("%s: Attempt to write eMMC start %d\n"
							, mmc_hostname(card->host), brq->cmd.arg);
						return 1;
					}
				}
			}
...
static int sd_blk_issue_rw_rq(struct mmc_queue *mq, struct request *rqc)
{
	...
	do {
		...
		case MMC_BLK_ECC_ERR:
			...
			/* If singale block read still fails, we start to reinit card. */
			try_recovery++;
			if (try_recovery < 2 && card->do_remove == 0) {
				do_reinit = 1;
				goto recovery;
